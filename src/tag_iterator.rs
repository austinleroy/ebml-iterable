    ...
    TSpec: EbmlSpecification<TSpec> + EbmlTag<TSpec> + Clone
{
    ...
    fn peek_valid_tag_header(&mut self) -> Result<(u64, Option<TagDataType>, EBMLSize, usize), TagIteratorError> {
        ...
        let header = tag_parse::read_header::<TSpec>(available, current_offset)?
            .ok_or_else(|| TagIteratorError::UnexpectedEOF {
                tag_start: current_offset,
                tag_id: tag_parse::read_tag_id(available).map(|(id, _)| id),
                tag_size: None,
                partial_data: None,
            })?;
