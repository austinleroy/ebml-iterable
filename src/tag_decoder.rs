...
pub struct PositionedTag<TSpec> {
    /// The decoded tag.
    pub tag: TSpec,
    /// The tag's byte offset in the input stream.
    pub offset: usize,
}
...
///
/// The decoder leaves incomplete elements in the caller's [`BytesMut`] and returns `None` until more bytes arrive.
/// Call [`Self::decode_eof`] only when the input has permanently ended.
...
where
    TSpec: EbmlSpecification<TSpec> + EbmlTag<TSpec> + Clone,
{
    /// Creates a decoder and selects the master tags that should be emitted as [`Master::Full`].
    ...
    /// Selects validation errors that the decoder may ignore.
    ...
    /// Sets the largest allowed tag payload, or removes the limit with `None`.
    pub fn set_max_allowable_tag_size(&mut self, size: Option<usize>) {
        self.max_allowed_tag_size = size;
    }

    /// Returns the byte offset of the next unread element.
    pub fn position(&self) -> usize {
        self.position
    }

    /// Returns the next available tag, or `None` when the decoder needs more bytes.
    ...
    /// Finishes the stream and returns any remaining master end tags.
    ///
    /// This reports an unexpected end of file when `input` contains a partial element.
    ...
    /// Returns whether end-of-input processing and tag emission are complete.
    ...
    fn unexpected_eof(&self, input: &[u8]) -> TagIteratorError {
        let parsed_id = tag_parse::read_tag_id(input);
        let tag_id = parsed_id.map(|(id, _)| id);
        let size = parsed_id.and_then(|(_, id_len)| {
            tools::read_vint(&input[id_len..])
                .ok()
                .flatten()
